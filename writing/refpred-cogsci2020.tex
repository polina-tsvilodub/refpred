...
Participants completed 12 trials, completing exactly one trial in each condition (syntactic frame [subject vs. predicate], visual context [subordinate vs. basic], noun [subordinate vs. basic vs. \emph{one}]), the order of which was randomized.\footnote{Due to a coding error, participants saw a ba}
...
As a result, the referential -- predicative trade-off view would not expect comparison class inferences to differ across syntactic positions for the basic-N label, which is indeed what we found. 
At the same time, in the subordinate-level context, we did observe a higher rate of basic-level comparison class inferences for basic-Ns:  since the context makes the basic-N referentially uninformative, listeners may be reasoning that the usage of the N was to set the comparison class.
