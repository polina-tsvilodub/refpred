...
Participants completed 12 trials, completing a random set of trials that could vary by condition (syntactic frame [subject vs. predicate], visual context [subordinate vs. basic], and noun [subordinate vs. basic vs. \emph{one}]).
...
At the same time, in the subordinate-level context, we did observe a higher rate of basic-level comparison class inferences for basic-level nouns:  since the context makes the basic-level referentially uninformative, listeners may reason that the usage of the noun was to set the comparison class.
