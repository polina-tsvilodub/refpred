...
When interpreting a gradable adjective in a sentence like "That great dane is big" the listener needs to construct a felicitous comparison class for the adjective 'big' in order to infer the size of the target. Constructing the comparison class is an inferential problem, since the target might be conceptualized as a member of different levels of the taxonomy it belongs to.  
Syntactic cues like the position of the noun phrase in a sentence might help listeners restrict possible comparison classes upon hearing a sentence with a gradable adjective like "That great dane is big". 

We test this hypothesis in three experiments manipulating the syntax of a sentence that is used to describe the size of an object: the syntax is either "That NP is big (small)" (the NP serves as subject) or "That's a big (small) NP" (the NP serves as predicate). 

__We hypothesize that NPs used as subjects contribute to reference, whereas NPs used as predicates contribute to predication. In terms of gradable adjectve use, predication is tantamount to communicating the comparison class, i. e. restricting the possible comparison classes to the felicitous one. Given our experimental design, the felicitous comparison class is the basic-level category of the target, such that the felicitous NP used as predicate is the basic-level label of the target. In contrast, the NP used as subject restricts the comparison class less, and thus using both the basic-level and the subordinate labels of the target as this NP is felicitous.__


## Syntax Rating Experiment
The goal of the experiment was to obtain ratings of both syntactic conditions in direct comparison as descriptions of a target referent in context. The context picture was followed by a sentence providing the subordinate level label of the traget. Below the sentence, the picture of the target appeared and participants were asked: "How well does each of the sentences describe it?". Both sentences use the same NP, differing only in NP position.
...
__Expectation:__ In the congruent condition, _given sentences with subordinate noun phrases (e. g. great dane), we expect higher mean ratings of the Subject than the Predicate condition_. Sentences with basic-level noun phrases should elicit similarly high ratings, regardless of syntactic condition. In the incongruent condition, the prenominal subordinate condition is expected to elicit the highest rating, since it is the only sentence felicitous to describe the target. The endorsement wwill probably be lower than in the congruent condition. Predicative subordinate and both basic-level conditions are expected to elicit the lowest ratings. 
...
```{r}
# full data set
lm.fit.full <- lmer(response1 ~ condition * np * congruence + 
...
#b.lm.w_prior <- brm(resp_cat ~   0 + syntax + (1 + syntax || submission_id) + (1 + syntax ||picture), 
 #                 data = d_prod_main_responseCat, family = "bernoulli",
...
Participants are asked to paraphrase a sentence in a free production task: They see a context picture (basic-level or subordinate, balanced) and are told that they and a friend see another not pictured member of the group. The friend utters a sentence (one of the two syntactic frames), after which the question "What do you think your friend meant?" is presented. The NP of the uttered sentence is either underspecified ('one') or the subordinate target label (balanced within-subject). The paraphrase template is "It is big (small) relative to other __ ", blank to be filled in. There are congruent and incongruent trials in the subordinate-level context. 

__Between-subjects:__ syntactic condition
...
Given the data, we expect an interaction of the NP and the syntactic condition and a main effect of context, but no interaction of NP, condition and context. We fit a linear model of response predicted by the syntactic condition, the NP and their and interaction and the context, with random slope effect of condition and NP interaction by participant. This model converges and yield significant results for all predictors (p < 0.01).
``` {r}
#  congruent trials only
...
lm.infer.congr <- glmer(response_num ~ condition*NP + context_cond +
                         # (1 + condition*NP*context_cond || submission_id) +
                       #   (1 + condition*NP*context_cond || item),
                         #     (1 | submission_id) +
                          #     (1 | item) +
                           #    (0 + condition| submission_id ) +
                          #     (0 + NP| submission_id) +
                      #   (0 + context_cond | submission_id) +
                       #        (0 + condition:NP | submission_id) +
                       # (0 + condition:context_cond | submission_id) +
                       #  (0+NP:context_cond|submission_id)+
                        #       (0 + condition |item) +
                            #   (0 + NP | item) +
                        # (0 + context_cond | item) +
                      #   (0 + condition:NP | item) +
                     #   (0 + condition:context_cond | item) +
                     #    (0+NP:context_cond|item)+
                      #          (0 + condition:NP | item),
                       (0 + condition:NP | submission_id), 
                   #      (0 + condition:NP:context_cond | item),
                    #   + (0 + context_cond| submission_id) + (0 + context_cond|item), 
...
#lm.infer.congr.fixed <- gls(response_num ~ condition * NP + context_cond, 
                           # data = d_infer_congr)
#summary(lm.infer.congr.fixed)
```

``` {r}
# congruent basic context trials 
d_infer_congr_basic <- d_infer_main_responseCat %>% filter(congruence == -1 & context_cond == 1)
# full-sized model cannot be estimated, 
# excluding all random effects but the interaction yields estimates which fully separate data 
lm.infer.congr.basic <- glmer(response_num ~ condition*NP + (1 + condition*NP || submission_id) + (1 + condition*NP || item),  
                               # (1 | submission_id) +
                              #  (1 | item) +
                              #  (0 + condition| submission_id ) +
                              #  (0 + NP| submission_id) +
...
#lm.infer.congr.basic.fixed <- gls(response_num ~ condition*NP, data = d_infer_congr_basic)
#summary(lm.infer.congr.basic.fixed)
```
...
``` {r}
# subset congruent subordinate context trials
d_infer_congr_sub <- d_infer_main_responseCat %>% filter(congruence == "congr" & context_cond== "sub")
#lm.infer.congr.sub <- glmer(response_num ~ condition*NP + 
                         #     (1 | submission_id) +
                          #     (1 | item) +
                          #     (0 + condition| submission_id ) +
                          #     (0 + NP| submission_id) +
...
# simple gls model
lm.infer.congr.sub.fixed <- gls(response_num ~ condition*NP, data= d_infer_congr_sub)
summary(lm.infer.congr.sub.fixed)
```
