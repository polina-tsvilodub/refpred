...
``` {r}
# subsetting congruent data
# fails to converge 
d_infer_congr <- d_infer_main_responseCat %>% filter(congruence == "congr")
lm.infer.congr <- glmer(response_num ~ condition*NP + context_cond + 
                         #     (1 | submission_id) +
                          ...
                          #     (0 + condition |item) +
                            #    (0 + NP | item) +
                      #          (0 + condition:NP | item),
                       #(0 + condition:NP:context_cond | submission_id), 
...
``` {r}
# subset congruent basic context trials 
d_infer_congr_basic <- d_infer_main_responseCat %>% filter(congruence == "congr" & context_cond== "basic")
...
#lm.infer.congr.basic <- lmer(response_num ~ condition*NP,  
                               # (1 | submission_id) +
                              #  (1 | item) +
                              #  (0 + condition| submission_id ) +
                              #  (0 + NP| submission_id) +
