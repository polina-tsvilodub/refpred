...
d_modRef_filter300 <- read_csv("../../prolific/double-mod-pilot-2/double-mod-pilot-2-trials.csv") %>%
...
First, invalid responses are excluded. This is a minimal pre-processing step:  Excluded responses consist of massive misspellings, compare the referent to a size, or to an invalid subordinate category (reference failure). This excludes 14 responses (0.6%).
...
This would exclude 77 responses in total (3.2%). But we decided to **not** exclude ungrammatical responses.

```{r invalid2, echo=FALSE, eval=F}
...
```{r classify-strict, echo=FALSE, eval=FALSE}
...
```{r classify3way-strict, echo=FALSE, eval=FALSE}
...
```{r plot-strict, echo=FALSE, eval=FALSE}
##### Plots
...
```{r plot2-strict, echo = F, eval=FALSE}
...
contrasts(d_modRef_main_responseCat$syntax_dev)
contrasts(d_modRef_main_responseCat$trial_type_dev) <- contr.sum(2)

contrasts(d_modRef_main_responseCat$trial_type_dev)
contrasts(d_modRef_main_responseCat$adj_dev) <- contr.sum(2)
contrasts(d_modRef_main_responseCat$adj_dev) 
...
```{r}
critical_subj <- c(critical_subj = "Intercept - trial_type_dev1 + syntax_dev1 - syntax_dev1:trial_type_dev1= 0")
critical_pred <- c(critical_pred = "Intercept - trial_type_dev1 - syntax_dev1 + syntax_dev1:trial_type_dev1 = 0")

filler_subj <- c(filler_subj = "Intercept + trial_type_dev1 + syntax_dev1 + syntax_dev1:trial_type_dev1= 0")
filler_pred <- c(filler_pred = "Intercept + trial_type_dev1 - syntax_dev1 - syntax_dev1:trial_type_dev1 = 0")

syntax_critical <- c(syntax_critical = "2 * syntax_dev1 - 2 * syntax_dev1:trial_type_dev1 = 0")
syntax_filler <- c(syntax_filler = "2 * syntax_dev1 + 2 * syntax_dev1:trial_type_dev1 = 0")

subj_critical_filler <- c(subj_critical_filler = "-2*trial_type_dev1 - 2 * syntax_dev1:trial_type_dev1 = 0" )
pred_critical_filler <- c(pred_critical_filler = "-2*trial_type_dev1 + 2 * syntax_dev1:trial_type_dev1 = 0" )

brm_prereg_model_contrast_answers <- hypothesis(brm_prereg_model, c(critical_subj, critical_pred, syntax_critical,
                                 filler_subj, filler_pred, syntax_filler,
                                 subj_critical_filler, pred_critical_filler))
brm_prereg_model_contrast_answers
```
...
Exploratory model on both trial types with a fixed effect of adjective (big vs. small):
...
```{r adjective-FE-critical, message=FALSE, warning=FALSE}
