...
sim_data_fit <- function(seed, N) {
  ...
  update(predicted_fit,
         newdata = data,
         seed = seed,
      #   cores = 4, # cannot be included if %dopar% is used 
         silent = T,
         refresh = 0) %>%
    # extract posterior draws
    spread_draws(b_Intercept, b_syntax_dev1, b_trial_dev1, `b_syntax_dev1:trial_dev1`) %>%
...
registerDoParallel(cores = 2) # number of workers, specify 4 cores 

getDoParWorkers() # check how many workers foreach is going to use after setting desired number

analyse.power <- foreach(n.subj = seq(10, 20, by= 10), .combine = rbind, .multicombine = T) %dopar% {
...
write_csv(analyse.power, paste("results/direct_mod_power_analysis_final_", n_iter,  "iter_", n_sim, "sim.csv", sep=""), append = T, col_names = T)
