...
predicted_draws <- pilot_model %>%
  ...
  mutate(critical_subj = b_Intercept + b_syntax_dev1 - b_trial_dev1 - `b_syntax_dev1:trial_dev1`,
         ...
         syntax_critical = 2*b_syntax_dev1 - 2*`b_syntax_dev1:trial_dev1`,
...
# power for effect of syntax
sim2 %>%
  filter(term == "b_syntax_devpred") %>%
  mutate(check_syntax = ifelse(upper < 0, 1, 0)) %>%
  summarise(power_syntax = mean(check_syntax))

# power for interaction 
sim2 %>%
  filter(term == "b_syntax_devpred:trial_devcritical") %>%
  mutate(check_interaction = ifelse(((lower < 0) & (upper > 0)), 1, 0)) %>%
  summarise(power_interaction = mean(check_interaction))
```
...
``` {r}
sim2 %>% 
  filter((term == "b_Intercept" )| (term == "b_syntax_devpred") | (term == "b_trial_devcritical") | (term == "b_syntax_devpred:trial_devcritical")) %>%
  select(seed, term, estimate) %>%
  spread(key = term, value = estimate) -> sim_wide

sim_wide %>%
  mutate(syntax_critical = b_Intercept + b_trial_devcritical) %>%
  summarise(mean = mean(syntax_critical),
            lower = quantile(syntax_critical, probs = 0.025),
            upper = quantile(syntax_critical, probs = 0.975))
