...
# helper
get_unique_id <- function(id, N) {
  workerid <- paste(id, N, sep="_")
}
...
get_new_data <- function(N, pilot_model, pilot_data) {
  ...
  data <- add_predicted_draws(model=pilot_model,
                              newdata = new.data,
                                n = 1) 
  worker_nums <- rep(1:N, each = 8) # 8 is nr of trials per particiapnt
...
analyse_power %>% write_csv(paste("results/direct_mod_power_analysis_100000subj_2_", n_iter, "iter_", n_sim, "sim.csv", sep = ""))
analyse_power_summary %>% write_csv(paste("results/direct_mod_power_analysis_100000subj_2_", n_iter, "iter_", n_sim, "sim_summary.csv", sep = ""))
