...
rs.glm <- glm(response_num~syntax_contr * (NP_subVone + NP_basicVone) * context_sub,
    data = d_infer_main_responseCat2,
    family = 'binomial')

summary(rs.glm)

rs.glm0 <- glm(response_num~syntax_contr * (NP_subVone + NP_basicVone) * context_sub - (NP_subVone + NP_basicVone) : syntax_contr,
    data = d_infer_main_responseCat2,
    family = 'binomial')
...
rs.glm.noint <- glm(response_num~syntax_contr * (NP_subVone + NP_basicVone) * context_sub - syntax_contr : (NP_subVone + NP_basicVone) : context_sub,
    data = d_infer_main_responseCat2,
    family = 'binomial')
...
```{r}
rs.glmer <- glmer(response_num~syntax_sum * (NP_subVone + NP_basicVone) * context_sum + 
                    (1 + syntax_sum * (NP_subVone + NP_basicVone) * context_sum || submission_id)  + 
                    (1 + syntax_sum * (NP_subVone + NP_basicVone) * context_sum || item),
    data = d_infer_main_responseCat2, 
    family = 'binomial',
...
```{r}
rs.glmer.basicContext <- glmer(response_num~syntax_sum * (NP_subVone + NP_basicVone) + 
                    (1 | submission_id)  + 
                    (1 | item),
                    # (1 + syntax_sum * (NP_subVone + NP_basicVone) || submission_id)  + 
                    # (1 + syntax_sum * (NP_subVone + NP_basicVone) || item),
    data = d_infer_main_responseCat2 %>% 
...
```{r}
rs.glmer.basicContext.noOne <- glmer(response_num~syntax_sum * NP_basicVone + 
                    (1 | submission_id)  + 
                    (1 | item),
                    # (1 + syntax_sum * (NP_subVone + NP_basicVone) || submission_id)  + 
                    # (1 + syntax_sum * (NP_subVone + NP_basicVone) || item),
...
rs.glmer.subContext.noOne <- glmer(response_num~syntax_sum * NP_basicVone + 
                    (1 | submission_id),
                    #(1 | item),
                    # (1 + syntax_sum * (NP_subVone + NP_basicVone) || submission_id)  + 
                    # (1 + syntax_sum * (NP_subVone + NP_basicVone) || item),
...
``` {r lmer}
# dummy / contrast coded exploratory models 
...
#   summarize_sample_vector(lm.b.infer3way.by.target.fit.samples$basic_v_sub, 'basic_v_sub'),
#   summarize_sample_vector(lm.b.infer3way.by.target.fit.samples$basic_v_one, 'basic_v_one'),
#   summarize_sample_vector(lm.b.infer3way.by.target.fit.samples$context_one_v_mean, 'context_one_v_mean'),
...
#   summarize_sample_vector(lm.b.infer3way.by.target.fit.samples$syntax_basic_v_sub, 'syntax_basic_v_sub'),
...
lm.b.infer3way.by.target.fit.samples %>% 
  ...
  summarize(
    ...
    prob_gt_0 = mean(val > 0),
    prob_lt_0 = mean(val < 0)
  ) -> lm.b.infer3way.full.contrasts
...
write_csv(data.frame(lm.b.infer3way.full.contrasts) %>% 
          ...
          path = "./../../writing/R_results_TeX/expt3_3way_full_contrasts_revised2.csv")
```
...
rbind(
  ...
  summarize_sample_vector(lm.b.infer.2way.fit.samples$syntax_basic_v_sub, 'syntax_basic_v_sub'),
...
write_csv(
  data.frame(lm.b.infer2way.full.contrasts) %>% 
    mutate(Rowname = Parameter), 
          path = "./../../writing/R_results_TeX/expt3_2way_full_contrasts_revised2.csv"
)
```
...
# two-way analysis
lm.b.infer.fit.two.way2.item <- brm(response_num ~ syntax_sum + NP_subVone + NP_basicVone + context_sum  +
                        syntax_sum:NP_subVone + syntax_sum:NP_basicVone +
                        (1 + syntax_sum + NP_subVone + NP_basicVone + context_sum  +
