...
``` {r data}
# read the data
d_prod1 <- read_csv('./data/results_28_prereg-NP-prod-subj.csv')
                 
d_prod2 <- read_csv('./data/results_29_prereg-NP-prod-pred.csv') 
d_prod2 <- d_prod2[!(names(d_prod2) %in% c("X40"))]
d_prod <- rbind(d_prod1, d_prod2)
```
...
``` {r}
# 2 participants too many in prenominal condition, excluding last  : 1299, 1300
...
```{r filtering}
# main trials 
d_prod_main <- d_prod_cleanData  %>% filter((trial_name == "main1") | (trial_name == "main2")) %>%
...
d_prod_main_responseCat %>%
  ...
  geom_col(position = position_dodge(0.8), fill="white", color = 'black', size = 1) +
...
```
``` {r brm csv}
myTable_np = cbind(tibble(Rowname = row.names(summary(b.prod.lm)[["fixed"]])), 
                summary(b.prod.lm)[["fixed"]] %>% as_tibble())
write_csv(myTable_np, path = "R_results_TeX/np-prod-brm.csv", col_names = T)
